...
app.openapi(getStats, async (c) => {
  ...
  // Sidebar badge: open pipeline (upcoming + needing close-out), not lifetime total.
  const openStatusPlaceholders = CLOSEOUT_STATUSES.map(() => "?").join(", ");
  const appointments = await get<{ count: number }>(
    `SELECT COUNT(*) as count FROM appointments WHERE status IN (${openStatusPlaceholders})`,
    [...CLOSEOUT_STATUSES],
  );
