...
export async function createBookingLinkCheckout(
  ...
  const adjusted = await maybeGrossUpLineItems(lineItems, feePassthrough);
...
export async function finalizeBookingLinkCheckout(
  ...
  await run(
    `INSERT INTO payments (appointment_id, booking_link_id, stripe_checkout_session_id, stripe_payment_intent_id, amount, currency, type, status, fee_passthrough)
     VALUES (?, ?, ?, ?, ?, ?, ?, 'succeeded', ?)`,
    [aptId, link.id, session.id, piId, amountPaid, link.currency, paymentType, link.fee_passthrough ? 1 : 0],
  );
