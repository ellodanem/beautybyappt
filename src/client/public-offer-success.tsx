...
export function PublicOfferSuccessPage({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    ...
    api<{ appointment: {
      ...
      "GET",
      `/api/offer/public/${encodeURIComponent(slug)}/complete?session_id=${encodeURIComponent(sessionId)}`,
    )
      ...
      <PublicPageShell platform={platform} contentClassName="flex flex-1 items-center justify-center p-4">
        <Card className="w-full max-w-md">
          <CardContent className="space-y-2 pt-6 text-center">
            ...
            <a href={`/offer/${encodeURIComponent(slug)}`} className="text-sm text-primary underline">
              Back to booking
            </a>
          </CardContent>
        </Card>
      ...
      <div className="mx-auto max-w-md space-y-4 pt-8">
        ...
        <Card>
          ...
          <CardContent className="space-y-2 text-center text-sm">
            ...
            {appointment.offering_name && <p className="font-semibold">{appointment.offering_name}</p>}
